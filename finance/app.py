...
def index():
    ...
    for symbol, shares in owns.items():
        ...
        total += stockValue
        owns[symbol] = (name, shares, usd(price), usd(stockValue))
...
def buy():
    ...
    if not result:
        return apology("Sorry invalid symbol", 400)
    ...
    # if shares < 0:
    #     return apology("Number of shares can not be negative!", 400)
    # elif (shares % 1) != 0:
    #     return apology("Number of shares must be an integer!", 400)
    # elif shares.isnumeric() == False:
    #     return apology("Number of shares but a numeric positive integer!", 400)
